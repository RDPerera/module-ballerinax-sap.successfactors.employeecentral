...
3. In the search bar, type `Organizational Position Management` and select the corresponding scenario from the results.
...
To use the `sap.successfactors.ecpositionmanagement` connector in your Ballerina application, modify the `.bal` file as follows:

### Step 1: Import the module

Import the `sap.successfactors.ecpositionmanagement` module.

```ballerina
import ballerinax/sap.successfactors.ecpositionmanagement as ecpos;
```
...
```ballerina
ecpos:PositionWrapper position = check ecposClient->getPosition();
```
