...
3. In the search bar, type `Employee Cost Distribution Management` and select the corresponding scenario from the results.
...
To use the `sap.successfactors.ecalternativecostdistribution` connector in your Ballerina application, modify the `.bal` file as follows:

### Step 1: Import the module

Import the `sap.successfactors.ecalternativecostdistribution` module.

```ballerina
import ballerinax/sap.successfactors.ecalternativecostdistribution as ecaltcost;
```
...
```ballerina
ecaltcost:EmpCostDistributionWrapper empCostDistribution = check ecaltcostClient->getEmpCostDistribution();
```
