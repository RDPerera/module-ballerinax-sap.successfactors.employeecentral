...
3. In the search bar, type `Employee Payroll Time Management` and select the corresponding scenario from the results.
...
To use the `sap.successfactors.ecpayrolltimesheets` connector in your Ballerina application, modify the `.bal` file as follows:

### Step 1: Import the module

Import the `sap.successfactors.ecpayrolltimesheets` module.

```ballerina
import ballerinax/sap.successfactors.ecpayrolltimesheets as ecpayroll;
```
...
```ballerina
ecpayroll:EmployeeTimeSheetWrapper timeSheet = check ecpayrollClient->getEmployeeTimeSheet();
```
